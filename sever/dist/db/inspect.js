...
function printStatus() {
    ...
    const lowStock = productList.filter((product) => product.stock > 0 && product.stock <= product.reorderPoint).length;
    const outOfStock = productList.filter((product) => product.stock === 0).length;
    const totalStockValue = productList.reduce((sum, product) => sum + product.stock * product.price, 0);
...
function printProducts() {
    ...
    console.table(productList.map((product) => ({
        SKU: product.sku,
...
function printProductRow(productId) {
    const product = db.select().from(products).where(eq(products.id, productId)).get();
    if (!product)
        return;
    console.table([
...
function restockProduct(productId, amountInput) {
    if (!productId) {
...
function addProduct() {
    try {
        ...
        console.error(error instanceof Error ? error.message : error);
        console.log('Example: npm run db:add-product -- --name "Iced Americano" --sku SKU-2001 --category "Coffee & Tea" --price 3.5 --stock 50 --reorder 15 --cost 0.9');
        process.exitCode = 1;
    }
}
...
function printOrders() {
    ...
    console.table(orderList.map((order) => {
...
function printSalesReport(period, key) {
...
switch (command) {
    ...
    default:
        ...
        console.log("Available commands: status, products, product <id>, orders, restock <id> <amount>, add-product, delete-product <id>, sales-daily <YYYY-MM-DD>, sales-monthly <YYYY-MM>");
        process.exitCode = 1;
}
