import { PaymentRepository, paymentRepository } from "../repositories/paymentRepository.js";
...
export class PaymentService {
  ...
  async createPayment(data: { invoiceId: string; paymentMethod: string }, cashierId?: string): Promise<any> {
    ...
    await auditLogService.logAction(
      ...
      data.invoiceId,
      "PAYMENT_CREATED",
      `Created payment of amount ${invoice.amount} via ${data.paymentMethod}`
    );
  ...
  async verifyPayment(
    ...
    const updatedPayment = await this.paymentRepo.verifyPayment(
      id,
      reference,
      cashierId,
      verifiedAt
    ...
    await auditLogService.logAction(
      ...
      payment.invoiceId,
      "PAYMENT_VERIFIED",
      `Verified payment of amount ${payment.amount} with reference ${reference}`
    );

    return updatedPayment;
  }

  async cancelPayment(id: string, cashierId?: string): Promise<any> {
    const payment = await this.paymentRepo.findById(id);
    if (!payment) {
      throw new Error(`Payment with ID '${id}' not found`);
    }
    ...
    await auditLogService.logAction(
      ...
      payment.invoiceId,
      "PAYMENT_CANCELLED",
      `Cancelled payment of amount ${payment.amount}`
    );
  ...
  async checkExpirations(): Promise<void> {
    ...
    for (const payment of expiredPayments) {
      ...
      await auditLogService.logAction(
        ...
        payment.invoiceId,
        "PAYMENT_EXPIRED",
        `Payment of amount ${payment.amount} expired automatically`
      );
    }
  }
}
